...
import { Country, Currency, Store, Cashier } from '@app/_models';
import { Observable, Subscription } from 'rxjs';
import { startWith, map, shareReplay } from 'rxjs/operators';
import { MatDialog, MatDialogConfig } from '@angular/material/dialog';
import { DialogComponent } from '@app/shared/dialog/dialog.component';
import { CompanyService, StoresService, CashiersService } from '@app/services/index';
...
export class CompanyComponent implements OnInit {
  ...
  currencies: Currency[]=[{"n":"Q", "c":"GTQ"},{"n":"EUR", "c":"EUR"}];
  ...
  ngOnInit() {
    ...
    this.filteredCurrencies = this.companyForm.get('Currency').valueChanges
      .pipe(
        startWith(''),
        map(currency => typeof currency === 'string' ? currency : currency.n),
  ...
  private _filterCurrency(value: string): Currency[] {
    let filterValue: string = '';
    filterValue = value.toLowerCase();
    return this.currencies.filter(currency => currency.n.toLowerCase().indexOf(filterValue) === 0);
  }
  ...
  onSubmitCompany(){
    ...
    if (this.companyForm.touched){
      ...
      this.companyService.updateCompany(this.companyId, dataForm)
        .subscribe(
          response =>  {
            this.loading = false;
  ...
  onSubmitStores(){
    ...
    if (this.storeForm.touched){
      this.storeService.updateStores(this.storeForm.value)
        .subscribe(
          response =>  {
            this.loading = false;
            this.companyForm.markAsPristine();
      ...
      // this.fCashier.controls.forEach(element => {
  ...
  onSubmitCashiers(){
    ...
    this.storeService.updateStoreCashier(upStores)
      .subscribe(
        response =>  {
          if (response != null){
            this.cashierService.updateCashiers(upCashier)
              .subscribe(
                response =>  {
