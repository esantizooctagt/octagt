...
export class CustomerComponent implements OnInit {
  ...
  getErrorMessage(component: string) {
    ...
    if (component === 'Email'){
      return this.fCustomer.Email.hasError('pattern') ? 'Email invalid' :
        '';
    }
  }

  onValueChanges(): void {
    this.subsCustomer = this.customerForm.valueChanges.subscribe(val=>{
      if (val.Status === true) {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.customer.currentValue != undefined) {
      var spinnerRef = this.spinnerService.start("Loading Customer...");
      ...
      this.customer$ = this.customerService.getCustomer(customerResult.Customer_Id).pipe(
        tap(res => {
          if (res != null) {
            ...
            this.customerForm.setValue({
              ...
              Is_Exent: (res.Is_Exent === true ? true : false),
              Reason: res.Reason,
              Email: res.Email,
              Status: res.Status
            });
  ...
  onSubmit(){
    ...
    if (this.customerForm.touched){
      ...
      var spinnerRef = this.spinnerService.start("Saving Customer...");
      if (customerId !== '' && customerId !== null) {  
        ...
        this.customerSave$ = this.customerService.updateCustomer(customerId, dataForm).pipe(
          tap(res => { 
        ...
        this.customerSave$ = this.customerService.postCustomer(dataForm).pipe(
          tap(res => { 
            this.savingCustomer = true;
            this.spinnerService.stop(spinnerRef);
            this.customerForm.reset({Is_Exent:0, Status:1, CustomerId:'', CompanyId:'', Reason:'', Email:'', Postal_Code:'', State:'', Name:'', Phone:'', Tax_Number:'', Address:'', House_No:'', Country:''});
